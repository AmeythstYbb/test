...
t.appendIndexes = function (s) {
    this.indexes += this.get__() + s + this.get__();
}
t.getUniqueId = function () {
    return "MzData" + (MzData.nodeCounter++).toString(36);
};
...
MzData.prototype.initialize = function () {
    ...
    for (i in this.dataSource)a[a.length] = i;
    this.appendIndexes(a.join(_));
...
MzData.prototype.getNodeById = function (id) {
    ...
    if (reg.test(this.indexes)) {
        ...
        if (s = this.dataSource[s].getAttribute("index_" + this.hashCode))
            return this.nodes[s];
        else {/*System._alert("The node isn't initialized!");*/
            return null;
        }
    }
...
MzData.prototype._loadXmlNodeData = function (xmlNode, parentId) {
    ...
    for (var k, id, i = 0, data = {}, n = xmlNode.childNodes; i < n.length; i++) {
        if (n[i].nodeType == 1) {
            id = n[i].getAttribute("id") || this.getUniqueId();
            if (n[i].hasChildNodes()) {
                for (k = 0, nic = n[i].childNodes; k < nic.length; k++) {
                    if (nic[k].nodeType == 1) {
                        this._loadXmlNodeData(n[i], id);
                        break;
                    }
                }
            }
...
MzData.prototype.loadUlData = function (HtmlUL, parentId) {
    ...
    if ("object" == typeof ul) {
        var data = {};
        for (var i = 0, n = ul.childNodes; i < n.length; i++) {
...
MzDataNode.prototype.DTO = function (DataNodeClass, sourceIndex) {
    ...
    s = $.dataSource[n.sourceIndex = sourceIndex];
