...
(function ($) {
    ...
    //            var tar = arr[1].split('=');
    //            $.post(arr[0], {tar[0]:tar[1]},function (html) {
    //                $("#sidebar").find(".accordion").remove().end().append(html).initUI();
    //                $box.find("li").removeClass("selected");
    //                $a.parent().addClass("selected");
    //                navTab.closeAllTab()
...
(function ($) {
    var _op = {cursor: "move", sortBoxs: "div.sortDrag", replace: false, items: "> div", selector: "", zIndex: 1000};
    DWZ.sortDrag = {
        start: function ($sortBox, $item, event, op) {
            ...
            var itemPos = $item.position(), y = itemPos.top + ($item.height() / 2), x = itemPos.left + ($item.width() / 2);
...
$.iframeLoad = function (url, callback) {
    var $form = $('<form method="post" action="' + url + '" target="callbackframe" style="display: none"><button type="submit">submit</button></form>').appendTo("body"), $iframe = $("#callbackframe");
    if ($iframe.size() == 0) {
        $iframe = $('<iframe id="callbackframe" name="callbackframe" src="about:blank" style="display:none"></iframe>').appendTo("body")
    }
