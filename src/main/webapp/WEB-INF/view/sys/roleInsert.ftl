...
<div class="pageContent">
    <form method="post" action="/role/insert" class="pageForm required-validate"
...
<script type="text/javascript">
    ...
    <#if menuList?? && (menuList?size > 0)>
        <#list menuList as menu>
            menuData['${(menu.parentId?? && menu.parentId == 0)?string('-1',menu.parentId)}_${(menu.id)!}'] = "text:${(menu.parentId?? && menu.parentId == 0)?string('权限列表',menu.name)};";
        </#list>
    </#if>
//        checked:true;
