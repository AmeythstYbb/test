...
import com.zlycare.web.boss_24.beans.annotation.AmountProperty;
import com.zlycare.web.boss_24.beans.annotation.DateFormatProperty;
import com.zlycare.web.boss_24.beans.annotation.ImageUrlProperty;
...
import com.zlycare.web.boss_24.exception.IllegalNumberException;
import com.zlycare.web.boss_24.utils.core.DateUtil;
    ...
    private void AmountFormatter(Field field, Object obj) {
        if (field.isAnnotationPresent(AmountProperty.class)) {
            try {
                ...
                String getMethodName = this.generateMethodName(GET_STR, field);
                String setMethodName = this.generateMethodName(SET_STR, field);
                Long amount;
                Double amountD;
                ...
                if (amountFormatConstant.equals(AmountFormatConstant.AMOUNT_TRANSFER_DIVIDE_DOUBLE)) {
                    if (obj != null) {
                        Object sourceObject = obj.getClass().getMethod(getMethodName).invoke(obj);
                        if (sourceObject != null) {
                            String source = sourceObject.toString();
                            if (!StringUtils.isEmpty(source)) {
                                amountD = Double.parseDouble(source) / 100;
                                obj.getClass().getMethod(setMethodName, field.getType()).invoke(obj, amountD);
                            }
                        }
                    }
                }
                ...
                if (amountFormatConstant.equals(AmountFormatConstant.AMOUNT_TRANSFER_MULTIPLY_DOUBLE)) {
                    if (obj != null) {
                        Object sourceObject = obj.getClass().getMethod(getMethodName).invoke(obj);
                        if (sourceObject != null) {
                            String source = sourceObject.toString();
